# 🤖 SunFounder PiDog AI JARVIS Assistant

![Raspberry Pi](https://img.shields.io/badge/Platform-Raspberry%20Pi-red)
![Python](https://img.shields.io/badge/Language-Python-blue)
![AI Robotics](https://img.shields.io/badge/Field-AI%20Robotics-green)
![Status](https://img.shields.io/badge/Status-Active-brightgreen)

---

## 📖 Overview

The **SunFounder PiDog AI JARVIS Assistant** project is a Raspberry Pi powered robotics platform focused on embedded systems, AI assistant workflows, computer vision, robotics automation, and voice interaction.

The project combines robotics, Python automation, embedded Linux systems, servo control, AI assistant concepts, and modular subsystem design into a unified robotic platform.

---

## ✨ Features

- JARVIS-style AI assistant structure
- Voice command handling
- Robotics patrol behaviors
- Servo calibration workflows
- Object tracking concepts
- Camera integration
- Embedded Linux environment
- Modular Python subsystem design
- Multi-folder engineering project organization
- SSH and VNC remote workflows

---

## 🛠 Hardware Used

- Raspberry Pi platform
- SunFounder PiDog robotics kit
- Servo motors
- Camera module
- Microphone input
- Speaker/audio system
- Wireless networking
- Embedded Linux environment

---

## 🔊 System Workflow

```text
Voice Input
→ Voice Assistant Processing
→ AI Behavior Logic
→ Robotics Control
→ Servo Movement
→ Camera / Vision Feedback
→ System Response
```

---

## 🧠 Engineering Concepts Demonstrated

- Embedded Linux Systems
- Robotics Integration
- AI Assistant Workflows
- Python Automation
- Computer Vision
- Voice Interaction Systems
- Servo Calibration
- Multi-Subsystem Integration
- Remote System Development
- Hardware/Software Debugging

---

## 📂 Repository Structure

```text
SunFounder-PiDog-AI-JARVIS-Assistant/

├── AI/
├── Camera/
├── Documentation/
├── Images/
├── Python_Code/
├── Servo_Calibration/
├── Videos/
├── Voice_Assistant/
└── README.md
```

---

## ⚙️ Development Stages

### Stage 1 — Servo Calibration
Initial servo alignment, zeroing, and movement testing.

### Stage 2 — Robotics Control
Basic movement scripting and patrol workflow development.

### Stage 3 — Voice Assistant
Voice command handling and JARVIS assistant workflow structure.

### Stage 4 — AI Integration
AI behavior systems and local assistant interaction concepts.

### Final System
Integrated robotics platform combining AI workflows, servo control, voice interaction, and camera systems.

---

## 📊 Python & System Development

Python scripts were used for:

- Robotics control
- Patrol behaviors
- Voice assistant workflows
- AI interaction systems
- Servo calibration
- Camera and object tracking concepts
- Embedded Linux automation

---

## 📷 Project Photos

### PiDog Hardware Platform
![PiDog](Images/pidog_main.jpg)

### Servo Calibration Workflow
![Servo Calibration](Images/servo_calibration.jpg)

### Camera and Vision System
![Camera System](Images/camera_system.jpg)

---

## 🎥 Demo Video

[Watch Project Demo](Videos/pidog_demo.mp4)

---

## 🚀 Future Improvements

- Wake-word activation
- Local LLM integration
- Advanced computer vision
- Autonomous navigation
- Real-time conversational AI
- Mobile app integration
- Web dashboard monitoring
- Advanced robotics behaviors

---

## 👨‍💻 Author

**Philip Perry**  
Bachelor of Science in Electrical Power Engineering Technology @ University of Houston  
Minor in Computer Science Engineering Technology  
U.S. Army Veteran & Former Sergeant
